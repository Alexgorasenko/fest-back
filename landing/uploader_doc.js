...
module.exports = async ({file, folder='docs', base64Data, userFileName}) => {
    ...
    try {
        ...
        switch (true) {
            ...
            case (!!(baseMime && baseMime.includes('sheet'))):
                ...
                break;
            case (!!(baseMime && baseMime.includes('msword'))):
                mime = 'application/msword'
                break;
            case (!!(baseMime && baseMime.includes('document'))):
        ...
        const typeFile = stypeFile === 'vnd.openxmlformats-officedocument.spreadsheetml.sheet' ? 'xlsx' : ['vnd.openxmlformats-officedocument.wordprocessingml.document','msword'].includes(stypeFile) ? 'doc' : stypeFile;
        console.log('typeFile', typeFile, mime);

        if (!typeFile || !validMimeTypes.includes(typeFile.toLowerCase())){
            return {success: false, msg: 'Неверный формат загружаемого файла',  errorStatus: 400}
        }
