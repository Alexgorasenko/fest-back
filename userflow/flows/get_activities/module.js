...
module.exports = async (req, item) => {
    try {
        ...
        const reducedActs = activities.filter(act => !act.participants || !act.participants.nominations || !act.participants.nominations.length || !!act.participants.nominations.find(nid => nid.toString() === nominationId.toString())).reduce((acc, cur) => {
            ...
            if (report && report.reportData) {
                ...
                if (studCount) {
                    ...
                    for (let level of querynom.levels) {
                        ...
                        acc.summary.totalScores += queryStuds ? Math.min((levelCount / queryStuds * 10), 10) : 0
                        acc.summary.activitiesScores += queryStuds ? Math.min((levelCount / queryStuds * 10), 10) : 0
            ...
            if (actScores && (!cur.isExtra || !festival.finishedVideoActivityId || cur._id.toString() !==  festival.finishedVideoActivityId.toString())) {
                acc.summary.activitiesReported += 1;
                acc.summary.finishButtonEnabled = acc.summary.activitiesReported >= (+festival.minCountFinishedReports || 4);
            }
            acc.activities.push({...outAct, activityScore: +(actScores.toFixed(1)) || null})
            return acc
        }, {
            "festivalId": festival._id,
            "queryId": query._id,
        ...
        return {
            success: true,
            data: {...reducedActs, summary: {...reducedActs.summary, activitiesScores: +reducedActs.summary.activitiesScores.toFixed(1), totalScores: +reducedActs.summary.totalScores.toFixed(1)}}
        }
